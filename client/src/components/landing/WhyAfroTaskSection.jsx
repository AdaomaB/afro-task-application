...
export default function WhyAfroTaskSection() {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-[#FB9E01] via-[#E68F07] to-[#C57810] w-full h-auto p-12 mx-2 lg:mx-8 py-12 lg:py-24 m-2 md:m-5 flex flex-col justify-center lg:flex-row items-center rounded-2xl md:rounded-3xl gap-4 overflow-visible relative shadow-2xl"
    >
      ...
      <div className="flex flex-col md:flex-col lg:justify-between md:justify-start justify-center items-center gap-4 w-full lg:w-2/5 relative z-0 mt-6 md:mt-0">
        ...
        <div className="self-start md:ml-20 mt-4">
          <Link to="/blogs">
            <button className="text-black bg-white flex flex-row md:w-[200px] lg:justify-center items-center py-2 px-6 md:py-2 md:px-8 rounded-2xl md:rounded-3xl shadow-lg text-base md:text-xl font-semibold group transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-xl">
              Read More{" "}
              <IoIosArrowForward className="font-semibold text-xl md:text-2xl transition-transform duration-300 group-hover:translate-x-2" />
            </button>
          </Link>
        </div>
      </div>
